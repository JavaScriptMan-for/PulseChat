  ...
  public async add_contact(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (!isHas)
        return res
          .status(404)
          .json({ message: "Контакта с таким user ID не существует" });
      ...
      if (candidate)
        return res.status(400).json({ message: "Контакт уже добавлен" });

      const unique_id = await nanoid();

      const my_contact = new Contact({
        my_userId: user.userId,
        userId,
        participant_1: user.name,
        participant_2,
        chat_id: unique_id,
      });
      ...
      res
        .status(200)
        .json({ message: "Контакт успешно добавлен", data: { userId } });
    } catch (error) {
      res.status(500).json({ message: "Ошибка сервера" });
      console.error(error);
    }
  ...
  public async delete_contact(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (!deletedContact)
        return res
          .status(400)
          .json({ message: "Ошибка при удалении контакта" });

      res
        .status(200)
        .json({ message: "Контакт удалён", data: { contactId: id } });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Ошибка сервера" });
    }
